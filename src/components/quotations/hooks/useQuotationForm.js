 ...
 useEffect(() => {
  const loadInitialData = async () => {
    try {
      ...
      // Customers will be fetched by the CustomerSelection component
      ...
      if (isEditMode) {
        ...
        setFormData({
          ...
          quotationId: quotationData.quotationId, // Keep the existing quotation ID
              ...
              const updatedItems = prevData.items.map((item) => {
                if (item.id === itemId) {
                  return {
                    ...
                    quantity: productFromBackend.quantity ?? '', // Preserve or empty
